...
function initialize_WALL_values_marker!(m, cache=X1CMPC.WALL_cache)
...
function initialize_WALL_contour_marker!(m, cache=X1CMPC.WALL_cache)
    m.header.frame_id = "wall"
    m.type = Marker[:LINE_STRIP]
    m.pose.orientation.w = 1.0
    m.scale.x = 0.1
...
function update_WALL_contour_marker!(m, q, cache=X1CMPC.WALL_cache)
    Y = cache.grid_knots[1]
    X = collect(range(-3, stop=5, length=17))
